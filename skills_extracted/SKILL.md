...
**Read this before planning any slide: `AFG_template.pptx` is a 102-slide
reference library, not just the base layouts (Title Page, Content
Slide_N columns, etc.).** Those base layouts are the structural skeleton
every slide needs — they are NOT the full set of design options. Sections
like "Diagrams and Charts," "Next Steps," "Numbers and factsheets," and
...
complete slide-by-slide index of all 102 is inline further down this
file** ("All 102 template slides" under "Layout and placeholder
...
repetitive. **Every one of the 102 slides is equally clonable — there is
no "verified" subset to stick to and no code restriction on any slide
number, including the Masterslides at the front.** This has been the
single most repeated correction on this skill — a deck that only ever
uses the base layouts, ignoring the rest of the reference slides, is
treated as a planning miss, not an acceptable default.
...
### The template's reference library — the complete map of all 102 slides

`AFG_template.pptx` (as authored) contains 102 example slides organized
into 13 real PowerPoint Sections (visible in PowerPoint's Slide Sorter
view) — this is the same underlying template file, so this map is exact,
not approximate.

**All 102 ARE directly editable — every single one, with no "verified"
whitelist and no slide number off-limits.** Two distinct mechanisms make
this true, and it matters to know which applies to what you're doing:
- **Structurally, they are built on a shared set of base PowerPoint
  layouts** (Section "Layout and placeholder reference" below) — a
  verifiable fact about the file, not a limitation this skill imposes. A
...
- **Functionally, `clone_slide` (see "Cloned visuals" below) lets you
  take ANY of the 102 directly and edit its actual content in place** —
  ...
  limited to extracting one piece. Nothing in the mechanism is specific
  to any particular slide — pick whichever reference slide actually fits
  the content, section by section. Do not restrict yourself to a small
  "known-good" set, and do not avoid a slide (including the Masterslides)
  just because it isn't on some prior verified list.

`_clear_existing_slides` strips every one of these 102 reference slides
...
and none of the 102 is ever copied wholesale into a generated deck
...
|---|---|---|
| Masterslides (1-23) | One demo of each base layout (title pages, chapter, content, split, full image, quote, thank-you) | Base-layout demos — clonable like any slide, but for a blank start you usually want the underlying layout directly. Reach here when you want the exact styled starting point of a specific base layout. |
| Corporate Introduction layouts (24-35) | Company/team intro slides: story, leadership, facts+icons | Reference for "About us"/intro sections — clone the icon-grid/facts slides directly, or map to existing layouts + `card_grid`/`accent_list`. |
| Executive Summary and resourcing (36-37) | Summary + resourcing slides | `card_grid` or `stat_row` for the summary; plain content for resourcing detail, or clone directly. |
| Strategy & Outcome (38-39) | Strategy/outcome statements | `process_steps` or `card_grid` depending on whether it's sequential or parallel, or clone directly. |
| Quarterly Roadmap (40-41) | Quarter-by-quarter plan | `process_steps`, or clone the roadmap slide directly if it's proportional rather than sequential. |
| Brands (42-45) | Logo/brand asset slides | Clone directly when the user wants a logo wall; otherwise a style reference. |
| Text Content (46-61) | Plain text/paragraph slide variants | Covered by existing `Content Slide_N columns` + plain `"bullets"`, or clone a specific variant directly. |
| Text and images (62-73) | Text paired with images, various ratios | Covered by `Split Content image` — pick the variant by dark/light, or clone a specific text+image composition directly. |
| Table Slides (74-80) | Tables and timetables | No native table support yet — clone the closest table/timetable/timeline slide directly and replace its text, fall back to `card_grid` (one card per row), or tell the user tables aren't supported, rather than improvising. |
| Organigram Slides (81-84) | Org charts / hierarchy | Clone the closest organigram directly, or use `card_grid` (one card per role/box) / nested `process_steps` for a reporting chain. |
| Numbers and factsheets (85-87) | KPI/stat callouts | Clone the factsheet/numbers slide directly, or use `stat_row` for headline numbers and `card_grid` when each number needs its own descriptive card. |
| Diagrams and Charts (88-97) | Circle segments (88-89), generic diagrams, icon grids, bar/keynumber charts | Slides 88/89 (Circle Segments) are strong clone candidates for proportional/parallel breakdowns. The chart slides (94, 97) are native charts — model your own chart on them, don't clone a chart object. |
| Next Steps (98-102) | Numbered step chains, various layouts | Clone the closest Next-Steps slide directly for a hand-designed step chain, or use `process_steps` for a plainer sequence. |

**In practice: sections 24-102 are where the genuinely useful
illustrations to clone live (Corporate Introduction, Diagrams and Charts,
Next Steps, Numbers and factsheets especially).** If a request doesn't
map to anything distinctive above, it's normal content covered by the
base layouts directly — this table exists to catch the cases where a
hand-designed illustration would look better than this skill's own
procedural approximation of the same idea. Every row is fully clonable;
...
|---|---|
| A phased plan or proportional/parallel breakdown (rollout stages, capability pillars, budget split) | **`clone_slide`** of a Circle Segments slide (88 or 89) — see "Cloned visuals" below |
| A sequential chain of numbered steps that needs a hand-designed look, not just `process_steps` | **`clone_slide`** of a Next Steps slide (98-102) |
...
### All 102 template slides — the complete list, right here in this file
...
three-column format as the base-layout table further down this section.
...
of it). Only fall back to the base-layout table beneath this one,
...
**Every single one of these 102 is clonable — there is no "verified"
subset and no code-level restriction on any slide number, including the
Masterslides (1-23).** Where a row recommends using the base layout
directly instead, that's a note about what usually looks best, not a
barrier the script enforces — cloning any row works exactly like any
other if you have a real reason to. Reach for whichever reference slide
genuinely fits the content; don't reuse the same one or two slides for
everything, and don't avoid a slide just because it isn't on some prior
known-good list.
