...
# from one of the template's 176 reference slides, verbatim, into a new
...
def clone_reference_slide(prs, template_path, source_slide_index,
    ...
    genuinely using one of the reference library's 176 slides AS a slide,
...
def amend_deck(input_path, plan, output_path, reference_template_path=None):
    ...
    176-slide reference library) -- required only if any edit or appended
...
def check_no_reference_library_usage(prs):
    ...
    practice, repeatedly, even after the 176-slide reference table was
    added directly to this skill's core instructions -- a deck built
    using only the ~34 base layouts, with the far larger reference
    ...
    return [
        ...
        f"check the full 176-slide reference table again before treating "
        f"this as correct. A deck this size using only the ~34 base "
        f"layouts, with the reference library never touched at all, is "
        f"the exact failure mode this check exists to catch -- it has "
        f"happened before even with the table already available."
    ]
